<template>
    ...
    <h1 class="font-bold text-2xl my-4 text-center text-blue-600">{{ $t('content.invoice') }}</h1>
    <hr class="mb-2">
    <div class="flex justify-between mb-6">
        <div class="text-gray-700">
            <div>{{ $t('content.date') }}: {{ format(new Date(state?.createdAt || '01/01/2000'),"dd/MM/yyyy") }}</div>
            <div>{{ $t('content.invoiceId') }}: {{ state?.id }}</div>
        </div>
    </div>
    <div class="mb-4">
