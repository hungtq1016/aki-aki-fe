...
<template>
    <form @submit.prevent="uploadFile">
        <div class="mb-4 flex items-center gap-3">
            ...
            <div><span class="mb-1.5 font-medium text-black-900 dark:text-gray-50">Edit your photo</span><span
                    class="flex gap-2.5">
                    <button @click="removeImage" type="button" class="text-sm font-medium hover:text--black-900"> Delete
                    </button><button class="text-sm font-medium hover:text--black-900"> Update
                    </button></span></div>
        </div>
        <div id="FileUpload"
            ...
            <div class="flex flex-col items-center justify-center space-y-3">
                ...
                <p class="text-sm font-medium"><span class="text-black-900">Click to upload</span> or drag
                    and drop </p>
            ...
            <div v-if="hasImage" class="absolute inset-0 bg-white">
                ...
                <img :src="isReview ? imageUrl : imageBuilderUrl(imageUrl) " alt="review-image" class="w-full h-full object-cover" />
            </div>
        </div>
...
async function uploadFile() {
    ...
    upload<TFileResponse>('/api/images', formData).then(response => {
