...
<template>
  <FormLayout :submit="submit">
    <FormItem>
      <FormGroup :has-error="[Boolean(errorFields?.roleId?.length)]">
        <template #heading>
          {{ $t('form.role') }}
        </template>
        <template #content>
          <FormSelect v-model="state.roleId" :list="roles" :has-error="Boolean(errorFields?.roleId?.length)" :placeholder="$t('form.place_holder.role_id')">
            {{ $t('form.role') }}
          </FormSelect>
        </template>
      </FormGroup>
            ...
            <li v-for="(permission, index) in permissions" :key="index">
              <label :for="`checkbox-${index}`">
                <div class="border border-gray-100 relative dark:border-zinc-900 rounded-md">
                  <input :checked="isChecked(permission)" @change="togglePermission(permission)" :disabled="state.roleId === '-1'"
...
import { isChecked, permissions, togglePermission, submit, rules, state } from '../services/logictics/assignment'
