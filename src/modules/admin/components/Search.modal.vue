...
<template>
              ...
              <DialogPanel
                ...
                <div class="w-full">
                  <div class="inline-flex w-full flex-col justify-center relative text-gray-500">
                    <div class="flex items-center gap-x-2 rounded border border-zinc-200 ">
                      <button type="button" class="focus:outline-none">
                        <MagnifyingGlassIcon class="w-5 h-5 ml-4" />
                      </button>
                      <input type="text" v-model="search"
                        class="p-2 w-full bg-zinc-200 focus:bg-white focus:outline-none focus:ring-0 focus:border-transparent rounded"
                    ...
                    <ul class="bg-white divide-y-[1px] divide-gray-500 w-full mt-2 ">
                      <li v-for="item in updatedData" :key="item._id"
                        ...
                        <a href="#" class="justify-between flex">
                          ...
                          <RouterLink to="#" class="capitalize text-sky-700">{{ item._index }}</RouterLink>
                        </a>
                      </li>
                    </ul>
                  </div>
  ...
  import type { IHit, IRootObject } from '@/core/models/interface';
  ...
  const fetch = async () => {
    ...
    auth: {
      username: 'elastic',
      password: 'MuO8YZRqIfKOF2dq0of5vpSy'
    },
    headers: {
