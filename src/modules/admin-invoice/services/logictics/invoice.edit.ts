...
watch(() => selectedServices.value, () => {
    state.value.total = selectedServices.value.reduce((total: number, service: TServicePrice) => total + Math.round(service.price), 0)
    state.value.tax = 0.1 * state.value.total
}, { deep: true })
