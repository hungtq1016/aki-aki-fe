...
import { rules } from '../services/logictics/tag'
import { useAsyncValidator } from '@vueuse/integrations/useAsyncValidator.mjs'

import { get } from '@/core/services/helpers/request.helper'

import type { TTagResponse, TTag } from '../models/type'
...
const stateUpdate: Ref<TTag> = ref({} as TTag)

const { pass, errorFields } = useAsyncValidator(stateUpdate, rules)

const fetchTag = async (): Promise<void> => {
  get<TTagResponse>('/api/tags/' + route.params.id).then((response) => {
    if (response?.data) {
...
const update = async () => {
  const data = await put<any, any>('/api/tags/' + stateUpdate.value.id, stateUpdate.value)
  if (data?.data) {
