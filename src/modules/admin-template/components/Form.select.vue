...
<template>
  <div>
    ...
    <div class="dark:bg-zinc-950 relative z-20 bg-white">
      ...
      <select :disabled="list?.length === 0" v-model="model"
       :class="{ '!border-red-600': hasError }" 
      class="dark:border-zinc-950 dark:bg-zinc-900 dark:text-gray-50 
      text-gray-900 w-full rounded-lg border border-gray-50 bg-transparent appearance-none px-12 py-3 font-normal outline-none transition 
      disabled:cursor-default disabled:!bg-gray-100 disabled:!text-gray-600 dark:disabled:!bg-stone-950 dark:disabled:!text-gray-50">
        ...
        <option v-for="(data, index) in list" :key="index" class="text-gray-900 dark:text-gray-100" :value="data.id">{{
          data.title }}</option>
      </select>
