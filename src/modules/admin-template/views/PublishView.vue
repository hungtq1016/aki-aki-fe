  ...
  <FormGroup :has-error="hasError">
    ...
    <template #content>
      <div class="flex items-center justify-between">
        <div>
          <Switch v-model="model.enable" :class="model.enable ? 'bg-green-200' : 'bg-red-200'"
            ...
            <span aria-hidden="true" :class="model.enable ? 'translate-x-9' : 'translate-x-0'"
        ...
        <div>
          <button :disabled="!pass" class="bg-cerulean-600 text-cerulean-100 flex justify-center rounded px-6 py-2 font-medium 
                  hover:bg-opacity-90 
                  dark:bg-cerulean-500 dark:text-cerulean-50
                  disabled:!bg-cerulean-100 disabled:!text-cerulean-600" type="submit">{{ $t('button.publish')
            }}</button>
        </div>
      </div>
    </template>
...
hasError.value.push(props.pass)
