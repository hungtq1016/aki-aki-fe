...
export const useAuthInfo = () => {
    ...
    const readAuthAsync = async (): Promise<TTokenResponse | undefined> => {
        const database: IDBDatabase = await openConnectionAsync();        
        try {
            const data: IDBRequest<TTokenResponse> = database.transaction(collectionName, "readonly").objectStore(collectionName).get(authKey);
    ...
    const resolveAuthRead = (data: IDBRequest<TTokenResponse>): Promise<TTokenResponse | undefined> => {
    ...
    const resolveAuthUpdate = async (objectStore: IDBObjectStore, newAuth: TTokenResponse): Promise<boolean> => {
